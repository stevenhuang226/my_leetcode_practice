...
int dijkstra(int nodes)
{
	...
	History *histories = malloc(nodes * sizeof(History));
	for (int i = 0; i < nodes; ++i) {
		histories[i].cost = INF32;
		histories[i].node = i;
	}
	min_heap_push(queue, &(histories[0]));

	while (queue->size > 0) {
		History *from = (History *)min_heap_pop(queue);
		uint32_t from_id = from->node;
		if (from->cost != dist[from_id]) {
			continue;
		}

		int next_edge = adj_head[from_id];

		while (next_edge >= 0) {
			int to_id = adj_to[next_edge];
			int weight = edge_weight[next_edge];
			next_edge = adj_next[next_edge];

			if (dist[to_id] <= dist[from_id] + weight) {
				continue;
			}

			dist[to_id] = dist[from_id] + weight;
			histories[to_id].cost = dist[to_id];
			min_heap_push(queue, &(histories[to_id]));
		}
	}
